...
interface ModuleShellProps {
  ...
  settingsHref?: string;
  renderSettings?: (props: { open: boolean; onOpenChange: (open: boolean) => void }) => React.ReactNode;
        ...
        <div className="px-2.5 py-1.5 border-t border-border/50">
          {settingsHref && !hasSettings ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-2.5 py-2.5 md:py-2 rounded-md text-xs transition-colors text-muted-foreground hover:text-foreground hover:bg-muted/50 w-full text-left"
              title={collapsed ? 'Settings' : undefined}
            >
              <Settings className="h-3.5 w-3.5 shrink-0" />
              {!collapsed && <span>Settings</span>}
...
export function ModuleShell({ moduleId, navItems, settingsHref, renderSettings, children }: ModuleShellProps) {
  ...
  const hasSettings = !!renderSettings;
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      {renderSettings?.({ open: settingsOpen, onOpenChange: setSettingsOpen })}
    </div>
  );
}
