import { PageHeader } from '@ccd/ui';
import { Clock } from 'lucide-react';

export default function TimeTrackingPage() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="Time Tracking"
        description="Track time spent on tasks and projects"
        breadcrumbs={[
          { label: 'Projects', href: '/projects' },
          { label: 'Time Tracking' },
        ]}
      />
      <div className="rounded-xl border border-dashed border-border/60 p-12 text-center">
        <Clock className="h-12 w-12 text-muted-foreground/20 mx-auto mb-4" />
        <p className="text-sm font-medium text-muted-foreground">Time tracking coming soon</p>
        <p className="text-xs text-muted-foreground/60 mt-1.5 max-w-sm mx-auto">
          Log hours, set estimates, and generate timesheets for billing and productivity insights.
        </p>
      </div>
    </div>
  );
}
